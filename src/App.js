...
function App() {
  ...
  const [startTime, setStartTime] = useState(Date.UTC(2020, 7, 22, 4, 0, 0, 0));
  ...
  useEffect(() => {
    ...
    if (window.web3) {
      interval = setInterval(
        (web3, address, presale, timer, redeemer, token) => {
    ...
    <ThemeProvider theme={theme}>
      ...
      {isActive & isEnded && (
        <Claimer
          ...
          maxShares={maxShares}
          hasSentToUniswap={hasSentToUniswap}
          hasIssuedTokens={hasIssuedTokens}
          hasSentEther={hasSentEther}
          finalEndTime={finalEndTime}
          accountRedeemable={accountRedeemable}
          accountClaimedLid={accountClaimedLid}
        />
      )}
      {isActive && !isEnded && (
        <>
